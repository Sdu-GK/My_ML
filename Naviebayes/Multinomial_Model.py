...
def trainNB0(trainMatrix, trainClass, ClassNum):
    Number = 0
    pDenom = np.zeros((ClassNum, 1))                                                    
    numTrainDocs = len(trainMatrix)						#计算训练的文档数目
    numWords = len(trainMatrix[0])                                              #统计训练数据所出现的所有单词种类数
    pNum = np.ones((ClassNum, numWords))                                        #拉普拉斯平滑化，分子加一，避免出现概率为零的情况
    
    for i in range(len(trainClass)):
        Number += sum(trainMatrix[i])                                           #计算整个训练样本的单词总数
    for i in range(ClassNum):                                                          
        pDenom[i] = numWords                                                    #拉普拉斯平滑化，分母加上文档中出现的单词种类数
        for j in range(numTrainDocs):
            if trainClass[j] == i:
                pNum[i] += trainMatrix[j]                                       #计算一个类别的文档中各个单词出现的次数
        pDenom[i] += sum(trainMatrix[j])                                        #计算分母，即一个类别的文档中总的单词数
    
    pClass = pDenom/Number							#计算文档属于各个类的概率，计算公式：P(C) = 类C下单词总数/整个训练样本的单词总数                                                            
        	                                       
    pVect = np.log(pNum/pDenom)							#取对数，防止下溢出，概率公式：P（word|C）= (类C下单词word在各个文档中出现的次数之和+1)/类C下单词总数+训练样本中出现的单词种类数                    
